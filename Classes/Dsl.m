...
- (Dsl *) init
{
  ...
  [self bindName:@"set-string"  toTarget:self andSelector:@selector(setString:)];
  [self bindName:@"set-integer" toTarget:self andSelector:@selector(setInteger:)];
  [self bindName:@"set-boolean" toTarget:self andSelector:@selector(setBoolean:)];
...
- (DslBoolean*) stringEqual:(DslCons*)args
{
  if (args == nil) return [DslBoolean withFalse];
  if ([self internalLength:args] != 2) return [DslBoolean withFalse];
